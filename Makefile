...
run-system-test: fmt vet
	PATH="$(PROJECT_DIR)/bin:${PATH}" PLATFORM_DESTINATION_IP=`docker inspect ${PLATFORM_CLUSTER_NAME}-control-plane | grep '"IPAddress": "172' | awk -F '"' '{print $$4}'` go run ${GINKGO} -v ${GINKGO_FLAGS} -p --output-interceptor-mode=none --coverprofile cover.out ./test/system/
...
run-git-integration-test: fmt vet ## Runs the integration test suite for the Git client
	...
	go run ${GINKGO} ${GINKGO_FLAGS} --coverprofile cover.out ./test/git/
