...
func (r *PromiseReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.createPromiseControllerResource(ctx, promise, rrCRD, rrGVK, logger); err != nil {
		logger.Error(err, "Error creating Works")
		return ctrl.Result{}, err
	}
...
func (r *PromiseReconciler) createPromiseControllerResource(ctx context.Context, promise *v1alpha1.Promise, rrCRD *apiextensionsv1.CustomResourceDefinition, rrGVK schema.GroupVersionKind, logger logr.Logger) error {
	pc := &v1alpha1.PromiseController{}
	pc.Name = promise.GetIdentifier()
	pc.Namespace = "default"
	pc.Labels = promise.GenerateSharedLabels()
	pc.Spec.ClusterSelector = promise.Spec.ClusterSelector
	pc.Spec.Pipelines = promise.Spec.XaasRequestPipeline
	pc.Spec.UID = string(promise.GetUID())
	pc.Spec.Group = rrGVK.Group
	pc.Spec.Version = rrGVK.Version
	pc.Spec.Kind = rrGVK.Kind
	pc.Spec.Plural = rrCRD.Spec.Names.Plural

	logger.Info("Creating Controller resource for promise")
	err := r.Client.Create(ctx, pc)
	if err != nil {
		if errors.IsAlreadyExists(err) {
			logger.Info("Work already exist", "controllerName", pc.Name)
			return nil
		}
		return err
	}
