...
func (r *PromiseReconciler) ensureDynamicControllerIsStarted(ctx context.Context, promise *v1alpha1.Promise, rrCRD *apiextensionsv1.CustomResourceDefinition, rrGVK *schema.GroupVersionKind, canCreateResources *bool, logger logr.Logger) error {
	...
	// The Dynamic Controller needs to be started once and only once per Promise.
	if r.dynamicControllerHasAlreadyStarted(promise, logger) {
		...
		if dynamicController.GVK != nil && dynamicController.CRD != nil {
			oldGVR := rrGVRFromCRD(dynamicController.CRD, dynamicController.GVK)
			if oldGVR != rrGVR {
				if err := r.DynamicController.Deregister(ctx, oldGVR); err != nil {
					return err
				}
			}
		}
		...
		handler := r.dynamicControllerHandler(dynamicController)
		return r.DynamicController.Register(ctx, rrGVR, handler, childGVRs...)
	}
...
func (r *PromiseReconciler) deregisterDynamicController(ctx context.Context, promise *v1alpha1.Promise, logger logr.Logger) error {
	if r.DynamicController == nil {
		return nil
	}
	...
	if dynamicController.GVK != nil && dynamicController.CRD != nil {
		if err := r.DynamicController.Deregister(ctx, rrGVRFromCRD(dynamicController.CRD, dynamicController.GVK)); err != nil {
			return err
		}
	}

	delete(r.StartedDynamicControllers, promise.GetDynamicControllerName(logger))
	return nil
}
...
func (r *PromiseReconciler) deletePromise(o opts, promise *v1alpha1.Promise) (ctrl.Result, error) {
	...
	if err := r.deregisterDynamicController(o.ctx, promise, o.logger); err != nil {
		return ctrl.Result{}, err
	}
